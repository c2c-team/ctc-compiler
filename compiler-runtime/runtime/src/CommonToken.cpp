...
CommonToken::CommonToken(std::pair<TokenSource *, CharStream *> source, size_t type, size_t channel,
...
std::string CommonToken::toString(Recognizer *r) const
{
    ...
    ss << "[@" << symbolToNumeric(getTokenIndex()) << "," << symbolToNumeric(_start) << ":"
       << symbolToNumeric(_stop) << "='" << txt << "',<" << typeString << ">" << channelStr << ","
