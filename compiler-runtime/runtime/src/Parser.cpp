...
void Parser::TraceListener::visitTerminal(tree::TerminalNode *node)
{
    std::cout << "consume " << node->getSymbol() << " rule "
              << outerInstance->getRuleNames()[outerInstance->getContext()->getRuleIndex()]
              << std::endl;
}
...
const atn::ATN &Parser::getATNWithBypassAlts()
{
    ...
    return *cache->map
                .insert(
                    std::make_pair(std::vector<int32_t>(serializedAtn.begin(), serializedAtn.end()),
                                   std::move(atn)))
                .first->second;
}

tree::pattern::ParseTreePattern Parser::compileParseTreePattern(const std::string &pattern,
...
void Parser::enterRecursionRule(ParserRuleContext *localctx, size_t state, size_t /*ruleIndex*/,
                                int precedence)
{
    setState(state);
...
bool Parser::isExpectedToken(size_t symbol)
{
           ...
           following.contains(Token::EPSILON))
    {
        ...
        const atn::RuleTransition *rt =
            static_cast<const atn::RuleTransition *>(invokingState->transitions[0].get());
    ...
    if (profile)
    {
        if (!is<atn::ProfilingATNSimulator *>(interp))
        {
            setInterpreter(new atn::ProfilingATNSimulator(
                this)); /* mem-check: replacing existing interpreter which gets deleted. */
        }
    }
