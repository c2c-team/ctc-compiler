    ...
    namespace misc
    {
        ...
        class ANTLR4CPP_PUBLIC MurmurHash final
        {
          ...
          public:
            ...
            template <typename T> static size_t update(size_t hash, const T *data, size_t size)
            {
            ...
            /// </summary>
            /// <param name="hash"> the intermediate hash value </param>
            /// <param name="entryCount"> the number of calls to update() before calling finish()
            /// </param> <returns> the final hash result </returns>
            static size_t finish(size_t hash, size_t entryCount);

            /// Utility function to compute the hash code of an array using the MurmurHash3
            /// algorithm.
            ///
