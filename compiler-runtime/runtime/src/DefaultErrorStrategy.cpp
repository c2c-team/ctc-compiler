...
{
    atn::ATNState *s =
        recognizer->getInterpreter<atn::ATNSimulator>()->atn.states[recognizer->getState()];
...
void DefaultErrorStrategy::reportNoViableAlternative(Parser *recognizer,
...
void DefaultErrorStrategy::reportFailedPredicate(Parser *recognizer,
...
Token *DefaultErrorStrategy::getMissingSymbol(Parser *recognizer)
{
    ...
    {
        tokenText =
            "<missing " + recognizer->getVocabulary().getDisplayName(expectedTokenType) + ">";
    }
    ...
    _errorSymbols.push_back(recognizer->getTokenFactory()->create(
        {current->getTokenSource(), current->getTokenSource()->getInputStream()}, expectedTokenType,
        tokenText, Token::DEFAULT_CHANNEL, INVALID_INDEX, INVALID_INDEX, current->getLine(),
...
misc::IntervalSet DefaultErrorStrategy::getErrorRecoverySet(Parser *recognizer)
{
    ...
    while (ctx->invokingState != ATNState::INVALID_STATE_NUMBER)
    {
        ...
        const atn::RuleTransition *rt =
            downCast<const atn::RuleTransition *>(invokingState->transitions[0].get());
