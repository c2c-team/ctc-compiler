...
Token *Lexer::emit()
{
    emit(_factory->create({this, _input}, type, _text, channel, tokenStartCharIndex,
                          getCharIndex() - 1, tokenStartLine, tokenStartCharPositionInLine));
    return token.get();
}
...
Token *Lexer::emitEOF()
{
    ...
    size_t line = getLine();
    emit(_factory->create({this, _input}, EOF, "", Token::DEFAULT_CHANNEL, _input->index(),
...
void Lexer::notifyListeners(const LexerNoViableAltException & /*e*/)
{
    ...
    std::string msg =
        std::string("token recognition error at: '") + getErrorDisplay(text) + std::string("'");
