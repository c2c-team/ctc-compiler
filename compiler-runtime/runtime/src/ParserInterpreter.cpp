...
ParserInterpreter::ParserInterpreter(const std::string &grammarFileName,
                                     ...
                                     const std::vector<std::string> &ruleNames, const atn::ATN &atn,
                                     TokenStream *input)
    : Parser(input), _grammarFileName(grammarFileName), _atn(atn), _ruleNames(ruleNames),
      _vocabulary(vocabulary)
{
    ...
    _interpreter = new atn::ParserATNSimulator(
        this, atn, _decisionToDFA, _sharedContextCache); /* mem-check: deleted in d-tor */
}
...
ParserRuleContext *ParserInterpreter::parse(size_t startRuleIndex)
{
    ...
    _rootContext =
        createInterpreterRuleContext(nullptr, atn::ATNState::INVALID_STATE_NUMBER, startRuleIndex);
...
void ParserInterpreter::enterRecursionRule(ParserRuleContext *localctx, size_t state,
...
void ParserInterpreter::visitState(atn::ATNState *p)
{
    ...
    switch (transition->getTransitionType())
    {
        ...
        case atn::TransitionType::NOT_SET:
            if (!transition->matches(static_cast<int>(_input->LA(1)), Token::MIN_USER_TOKEN_TYPE,
        ...
        case atn::TransitionType::PRECEDENCE: {
            if (!precpred(_ctx, static_cast<const atn::PrecedencePredicateTransition *>(transition)
                                    ->getPrecedence()))
            {
                throw FailedPredicateException(
                    ...
                    "precpred(_ctx, " +
                        std::to_string(
                            static_cast<const atn::PrecedencePredicateTransition *>(transition)
                                ->getPrecedence()) +
                        ")");
            }
        }
...
size_t ParserInterpreter::visitDecisionState(DecisionState *p)
{
    ...
    if (p->transitions.size() > 1)
    {
        ...
        if (decision == _overrideDecision && _input->index() == _overrideDecisionInputIndex &&
        ...
        {
            predictedAlt =
                getInterpreter<ParserATNSimulator>()->adaptivePredict(_input, decision, _ctx);
        }
    }
    return predictedAlt;
}
...
void ParserInterpreter::visitRuleStopState(atn::ATNState *p)
{
    ...
    const atn::RuleTransition *ruleTransition =
        static_cast<const atn::RuleTransition *>(_atn.states[getState()]->transitions[0].get());
    ...
    if (_input->index() == i)
    {
        ...
        if (is<InputMismatchException *>(&e))
        {
            ...
            _errorToken = getTokenFactory()->create(
                {tok->getTokenSource(), tok->getTokenSource()->getInputStream()}, expectedTokenType,
                tok->getText(), Token::DEFAULT_CHANNEL, INVALID_INDEX,
