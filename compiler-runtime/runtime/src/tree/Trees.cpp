...
ParserRuleContext *Trees::getRootOfSubtreeEnclosingRegion(ParseTree *t, size_t startTokenIndex,
                                                          size_t stopTokenIndex)
{
    ...
    {
        ParserRuleContext *r =
            getRootOfSubtreeEnclosingRegion(t->children[i], startTokenIndex, stopTokenIndex);
        if (r != nullptr)
        {
            return r;
        }
    ...
    if (is<ParserRuleContext *>(t))
    {
        ...
        {
            // note: r.getStop()==null likely implies that we bailed out of parser and there's
