...
bool ParseTreePatternMatcher::matches(ParseTree *tree, const std::string &pattern,
...
ParseTreeMatch ParseTreePatternMatcher::match(ParseTree *tree, const std::string &pattern,
                                              int patternRuleIndex)
{
    ParseTreePattern p = compile(pattern, patternRuleIndex);
    return match(tree, p);
...
ParseTreePattern ParseTreePatternMatcher::compile(const std::string &pattern, int patternRuleIndex)
{
    ...
    ParserInterpreter parserInterp(_parser->getGrammarFileName(), _parser->getVocabulary(),
                                   _parser->getRuleNames(), _parser->getATNWithBypassAlts(),
                                   &tokens);
    ...
    catch (std::exception & /*e*/)
    {
        std::throw_with_nested(
            RuntimeException("Cannot invoke start rule")); // Wrap any other exception.
#endif
    }
    ...
    std::map<std::string, std::vector<ParseTree *>> &labels)
{
    ...
    if (is<TerminalNode *>(tree) && is<TerminalNode *>(patternTree))
    {
        ...
        {
            if (is<TokenTagToken *>(t2->getSymbol()))
            { // x and <ID>
    ...
    for (auto chunk : chunks)
    {
        if (is<TagChunk *>(&chunk))
        {
            ...
            if (isupper(tagChunk.getTag()[0]))
            {
                ...
                tokens.emplace_back(
                    new TokenTagToken(tagChunk.getTag(), (int)ttype, tagChunk.getLabel()));
            }
            else if (islower(tagChunk.getTag()[0]))
            {
                ...
                tokens.emplace_back(new RuleTagToken(tagChunk.getTag(), ruleImaginaryTokenType,
                                                     tagChunk.getLabel()));
            }
            else
            {
...
std::vector<Chunk> ParseTreePatternMatcher::split(const std::string &pattern)
{
    ...
    for (size_t i = 0; i < ntags; i++)
    {
        // copy inside of <tag>
        std::string tag =
            pattern.substr(starts[i] + _start.length(), stops[i] - (starts[i] + _start.length()));
        ...
        if (i + 1 < ntags)
        {
            ...
            std::string text = pattern.substr(stops[i] + _stop.length(),
