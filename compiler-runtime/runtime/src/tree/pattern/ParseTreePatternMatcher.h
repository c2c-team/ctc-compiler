...
namespace antlr4
{
    namespace tree
    {
        namespace pattern
        {
            ...
            /// <seealso cref="#match"/> routines return a <seealso cref="ParseTreeMatch"/> object
            /// that contains the parse tree, the parse tree pattern, and a map from tag name to
            /// matched nodes (more below). A subtree that fails to match, returns with
            /// <seealso cref="ParseTreeMatch#mismatchedNode"/> set to the first tree node that did
            /// not match. <p/> For efficiency, you can compile a tree pattern in string form to a
            ...
            /// cref="ParseTreePattern#match"/> that are easy to use but not super efficient because
            /// they create new <seealso cref="ParseTreePatternMatcher"/> objects each time and have
            /// to compile the pattern in string form before using it. <p/> The lexer and parser
            /// that you pass into the <seealso cref="ParseTreePatternMatcher"/> constructor are
            /// used to parse the pattern in string form. The lexer converts the {@code <ID> =
            /// <expr>;} into a sequence of four tokens (assuming lexer throws out whitespace or
            /// puts it on a hidden channel). Be aware that the input stream is reset for the lexer
            /// (but not the parser; a <seealso cref="ParserInterpreter"/> is created to parse the
            /// input.). Any user-defined fields you have put into the lexer might get changed when
            /// this mechanism asks it to scan the pattern string. <p/> Normally a parser does not
            /// accept token {@code <expr>} as a valid
            /// {@code expr} but, from the parser passed in, we create a special version of
            /// the underlying grammar representation (an <seealso cref="ATN"/>) that allows
            /// imaginary tokens representing rules ({@code <expr>}) to match entire rules. We call
              ...
              public:
                ...
                class StartRuleDoesNotConsumeFullPattern : public RuntimeException
                {
                  public:
                    ...
                    StartRuleDoesNotConsumeFullPattern(StartRuleDoesNotConsumeFullPattern const &) =
                        default;
                ...
                /// cref="Lexer"/> and <seealso cref="Parser"/> object. The lexer input stream is
                /// altered for tokenizing the tree patterns. The parser is used as a convenient
                /// mechanism to get the grammar name, plus token, rule names.
                ...
                /// <param name="escapeLeft"> The escape sequence to use for escaping a start or
                /// stop delimiter.
                /// </param>
                /// <exception cref="IllegalArgumentException"> if {@code start} is {@code null} or
                /// empty. </exception> <exception cref="IllegalArgumentException"> if {@code stop}
                /// is {@code null} or empty. </exception>
                ...
                /// {@code tree} and return a <seealso cref="ParseTreeMatch"/> object that contains
                /// the matched elements, or the node at which the match failed.
                /// </summary>
                ...
                /// <seealso cref="ParseTreeMatch"/> object that contains the matched elements, or
                /// the node at which the match failed. Pass in a compiled pattern instead of a
