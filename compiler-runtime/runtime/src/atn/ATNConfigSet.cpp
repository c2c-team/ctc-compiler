...
ATNConfigSet::ATNConfigSet(const ATNConfigSet &other)
    : fullCtx(other.fullCtx), _configLookup(other._configLookup.bucket_count(),
                                            ATNConfigHasher{this}, ATNConfigComparer{this})
{
    addAll(other);
...
bool ATNConfigSet::add(const Ref<ATNConfig> &config, PredictionContextMergeCache *mergeCache)
{
    ...
    Ref<const PredictionContext> merged =
...
size_t ATNConfigSet::hashCode() const
{
    ...
    if (!isReadonly() || cachedHashCode == 0)
    {
        ...
        {
            cachedHashCode =
                31 * cachedHashCode + i->hashCode(); // Same as Java's list hashCode impl.
        }
