...
ParserATNSimulator::ParserATNSimulator(Parser *parser, const ATN &atn,
                                           ...
                                           ParserRuleContext *outerContext)
{
    ...
    std::cout << "adaptivePredict decision " << decision
              << " exec LA(1)==" << getLookaheadName(input) << " line " << input->LT(1)->getLine()
...
size_t ParserATNSimulator::execATN(dfa::DFA &dfa, dfa::DFAState *s0, TokenStream *input,
                                   size_t startIndex, ParserRuleContext *outerContext)
{

#if DEBUG_ATN == 1 || TRACE_ATN_SIM == 1
    std::cout << "execATN decision " << dfa.decision << ", DFA state " << s0->toString()
              << ", LA(1)==" << getLookaheadName(input) << " line " << input->LT(1)->getLine()
              << ":" << input->LT(1)->getCharPositionInLine() << std::endl;
#endif
    ...
    { // while more work
        ...
        if (D == ERROR.get())
        {
            ...
            NoViableAltException e =
                noViableAlt(input, outerContext, previousD->configs.get(), startIndex, false);
        ...
        if (D->requiresFullContext && _mode != PredictionMode::SLL)
        {
            ...
            reportAttemptingFullContext(dfa, conflictingAlts, D->configs.get(), startIndex,
                                        input->index());
            size_t alt =
                execATNWithFullContext(dfa, D, s0_closure.get(), input, startIndex, outerContext);
            return alt;
        }
...
dfa::DFAState *ParserATNSimulator::computeTargetState(dfa::DFA &dfa, dfa::DFAState *previousD,
                                                      size_t t)
{
    ...
    dfa::DFAState *D =
        new dfa::DFAState(std::move(reach)); /* mem-check: managed by the DFA or deleted below,
    ...
    if (state != D)
    {
        delete D; // If the new state exists already we don't need it and use the existing one
                  // instead.
    }
    return state;
}
...
void ParserATNSimulator::predicateDFAState(dfa::DFAState *dfaState, DecisionState *decisionState)
{
    ...
    std::vector<Ref<const SemanticContext>> altToPred =
                                                  ...
                                                  ParserRuleContext *outerContext)
{
    ...
    while (true)
    {
        ...
        if (reach == nullptr)
        {
            ...
            // will get error no matter what.
            NoViableAltException e =
                noViableAlt(input, outerContext, previous, startIndex, previous != s0);
            input->seek(startIndex);
            size_t alt =
                getSynValidOrSemInvalidAltThatFinishedDecisionEntryRule(previous, outerContext);
            if (alt != ATN::INVALID_ALT_NUMBER)
            {
                return alt;
            }
     ...
     */
    reportAmbiguity(dfa, D, startIndex, input->index(), foundExactAmbig, reach->getAlts(),
                    reach.get());

    return predictedAlt;
}

std::unique_ptr<ATNConfigSet> ParserATNSimulator::computeReachSet(ATNConfigSet *closure_, size_t t,
                                                                  bool fullCtx)
{
    ...
    if (t == IntStream::EOF)
    {
         ...
         */
        ATNConfigSet *temp =
            removeAllConfigsNotInRuleStopState(reach.get(), *reach == *intermediate);
        if (temp != reach.get())
    ...
    if (skippedStopStates.size() > 0 &&
        (!fullCtx || !PredictionModeClass::hasConfigInRuleStopState(reach.get())))
    {
...
#if DEBUG_ATN == 1 || TRACE_ATN_SIM == 1
    std::cout << "computeReachSet " << closure_->toString() << " -> " << reach->toString()
              << std::endl;
#endif
...
std::unique_ptr<ATNConfigSet> ParserATNSimulator::computeStartState(ATNState *p, RuleContext *ctx,
                                                                    bool fullCtx)
{
...
#if DEBUG_ATN == 1 || TRACE_ATN_SIM == 1
    std::cout << "computeStartState from ATN state " << p->toString()
              << " initialContext=" << initialContext->toString() << std::endl;
#endif
...
std::vector<Ref<const SemanticContext>> ParserATNSimulator::getPredsForAmbigAlts(
...
std::vector<dfa::DFAState::PredPrediction> ParserATNSimulator::getPredicatePredictions(
    const antlrcpp::BitSet &ambigAlts, const std::vector<Ref<const SemanticContext>> &altToPred)
{
    bool containsPredicate = std::find_if(altToPred.begin(), altToPred.end(),
...
size_t ParserATNSimulator::getSynValidOrSemInvalidAltThatFinishedDecisionEntryRule(
...
size_t ParserATNSimulator::getAltThatFinishedDecisionEntryRule(ATNConfigSet *configs)
{
    ...
    for (const auto &c : configs->configs)
    {
        if (c->getOuterContextDepth() > 0 ||
            (c->state != nullptr && c->state->getStateType() == ATNStateType::RULE_STOP &&
...
std::pair<ATNConfigSet *, ATNConfigSet *> ParserATNSimulator::splitAccordingToSemanticValidity(
    ATNConfigSet *configs, ParserRuleContext *outerContext)
{
    ...
    for (const auto &c : configs->configs)
    {
        if (c->semanticContext != SemanticContext::Empty::Instance)
        {
            bool predicateEvaluationResult =
                evalSemanticContext(c->semanticContext, outerContext, c->alt, configs->fullCtx);
    ...
    ParserRuleContext *outerContext, bool complete)
{
    ...
    for (const auto &prediction : predPredictions)
    {
        ...
        bool predicateEvaluationResult =
            evalSemanticContext(prediction.pred, outerContext, prediction.alt, fullCtx);
#if DEBUG_ATN == 1 || DFA_DEBUG == 1
        std::cout << "eval pred " << prediction.toString() << " = " << predicateEvaluationResult
                  << std::endl;
#endif
...
bool ParserATNSimulator::evalSemanticContext(Ref<const SemanticContext> const &pred,
                                             ParserRuleContext *parserCallStack, size_t /*alt*/,
                                             bool /*fullCtx*/)
{
    return pred->eval(parser, parserCallStack);
}

void ParserATNSimulator::closure(Ref<ATNConfig> const &config, ATNConfigSet *configs,
    ...
    if (config->state != nullptr && config->state->getStateType() == ATNStateType::RULE_STOP)
    {
        ...
        if (!config->context->isEmpty())
        {
            for (size_t i = 0; i < config->context->size(); i++)
            {
...
#if DEBUG_ATN == 1
                        std::cout << "FALLING off rule " << getRuleName(config->state->ruleIndex)
                                  << std::endl;
#endif
                ...
                closureCheckingStopState(c, configs, closureBusy, collectPredicates, fullCtx,
                                         depth - 1, treatEofAsEpsilon);
            }
            return;
        }
        else if (fullCtx)
...
void ParserATNSimulator::closure_(Ref<ATNConfig> const &config, ATNConfigSet *configs,
                                  ATNConfig::Set &closureBusy, bool collectPredicates, bool fullCtx,
                                  int depth, bool treatEofAsEpsilon)
{
    ...
    for (size_t i = 0; i < p->transitions.size(); i++)
    {
        ...
        bool continueCollecting =
            ...
            collectPredicates;
        Ref<ATNConfig> c =
            getEpsilonTarget(config, t, continueCollecting, depth == 0, fullCtx, treatEofAsEpsilon);
        if (c != nullptr)
        {
            int newDepth = depth;
                ...
                config->state->getStateType() == ATNStateType::RULE_STOP)
            {
                ...
                if (_dfa != nullptr && _dfa->isPrecedenceDfa())
                {
                    size_t outermostPrecedenceReturn =
                        downCast<const EpsilonTransition *>(t)->outermostPrecedenceReturn();
                ...
                configs->dipsIntoOuterContext = true; // TODO: can remove? only care when we add to
                                                      // set per middle of this method
            ...
            {
                // latch when newDepth goes negative - once we step out of the entry context we
                // can't return
                if (newDepth >= 0)
                {
                    newDepth++;
...
Ref<ATNConfig> ParserATNSimulator::getEpsilonTarget(Ref<ATNConfig> const &config,
                                                    const Transition *t, bool collectPredicates,
...
Ref<ATNConfig> ParserATNSimulator::actionTransition(Ref<ATNConfig> const &config,
...
Ref<ATNConfig> ParserATNSimulator::precedenceTransition(Ref<ATNConfig> const &config,
                                                        const PrecedencePredicateTransition *pt,
                                                        bool collectPredicates, bool inContext,
                                                        bool fullCtx)
{
#if DFA_DEBUG == 1
    std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->getPrecedence()
              << ">=_p"
              << ", ctx dependent=true" << std::endl;
    if (parser != nullptr)
    {
        std::cout << "context surrounding pred is "
                  << Arrays::listToString(parser->getRuleInvocationStack(), ", ") << std::endl;
    }
#endif
...
{
#if DFA_DEBUG == 1
    std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->getRuleIndex()
              << ":" << pt->getPredIndex() << ", ctx dependent=" << pt->isCtxDependent()
              << std::endl;
    if (parser != nullptr)
    {
        std::cout << "context surrounding pred is "
...
{
#if DFA_DEBUG == 1
    std::cout << "CALL rule " << getRuleName(t->target->ruleIndex) << ", ctx=" << config->context
              << std::endl;
#endif
...
NoViableAltException ParserATNSimulator::noViableAlt(TokenStream *input,
                                                     ParserRuleContext *outerContext,
                                                     ATNConfigSet *configs, size_t startIndex,
                                                     bool deleteConfigs)
{
    return NoViableAltException(parser, input, input->get(startIndex), input->LT(1), configs,
                                outerContext, deleteConfigs);
}
...
dfa::DFAState *ParserATNSimulator::addDFAEdge(dfa::DFA &dfa, dfa::DFAState *from, ssize_t t,
                                              dfa::DFAState *to)
{
#if DFA_DEBUG == 1
...
void ParserATNSimulator::reportAttemptingFullContext(dfa::DFA &dfa,
...
void ParserATNSimulator::reportContextSensitivity(dfa::DFA &dfa, size_t prediction,
                                                  ATNConfigSet *configs, size_t startIndex,
                                                  size_t stopIndex)
{
#if DFA_DEBUG == 1 || RETRY_DEBUG == 1
    ...
    if (parser != nullptr)
    {
        parser->getErrorListenerDispatch().reportContextSensitivity(parser, dfa, startIndex,
                                                                    stopIndex, prediction, configs);
    }
}

void ParserATNSimulator::reportAmbiguity(dfa::DFA &dfa, dfa::DFAState * /*D*/, size_t startIndex,
