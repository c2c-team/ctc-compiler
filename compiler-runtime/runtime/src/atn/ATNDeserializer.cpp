...
namespace
{
    ...
    ConstTransitionPtr edgeFactory(const ATN &atn, TransitionType type, size_t trg, size_t arg1,
        ...
        switch (type)
        {
            ...
            default:
                std::string message = "The specified state type " +
                                      std::to_string(static_cast<size_t>(type)) + " is not valid.";
    ...
    void deserializeSets(SerializedATNView data, int &p, std::vector<misc::IntervalSet> &sets)
    {
        size_t nsets = data[p++];
...
ATNDeserializer::ATNDeserializer(ATNDeserializationOptions deserializationOptions)
...
std::unique_ptr<ATN> ATNDeserializer::deserialize(SerializedATNView data) const
{
    ...
    {
        std::string reason = "Could not deserialize ATN with version" + std::to_string(version) +
                             "(expected " + std::to_string(SERIALIZED_VERSION) + ").";
        ...
        atn->grammarType == ATNType::PARSER)
    {
        ...
        for (std::vector<RuleStartState *>::size_type i = 0; i < atn->ruleToStartState.size(); i++)
        {
            ...
            {
                ConstTransitionPtr transition = atn->ruleToStartState[i]->removeTransition(
            ...
            matchState->addTransition(
                std::make_unique<AtomTransition>(bypassStop, atn->ruleToTokenType[i]));
        ...
        if (StarLoopEntryState::is(state))
        {
            ...
            if (StarBlockStartState::is(starLoopEntryState->transitions[0]->target))
            {
                checkCondition(downCast<LoopEndState *>(
                                   starLoopEntryState->transitions[1]->target) != nullptr);
