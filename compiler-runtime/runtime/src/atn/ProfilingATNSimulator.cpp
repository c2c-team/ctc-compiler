...
size_t ProfilingATNSimulator::adaptivePredict(TokenStream *input, size_t decision,
                                              ParserRuleContext *outerContext)
{
    auto onExit = finally([this]() {
        _currentDecision = 0; // Originally -1, but that makes no sense (index into a vector and
                              // init value is also 0).
    });
    ...
    _decisions[decision].SLL_MinLook = _decisions[decision].SLL_MinLook == 0
                                           ? SLL_k
    ...
    if (SLL_k > _decisions[decision].SLL_MaxLook)
    {
        ...
        _decisions[decision].SLL_MaxLookEvent = std::make_shared<LookaheadEventInfo>(
            decision, nullptr, alt, input, _startIndex, _sllStopIndex, false);
    }
    ...
    if (_llStopIndex >= 0)
    {
        ...
        _decisions[decision].LL_MinLook = _decisions[decision].LL_MinLook == 0
                                              ? LL_k
                                              : std::min(_decisions[decision].LL_MinLook, LL_k);
        if (LL_k > _decisions[decision].LL_MaxLook)
        {
            _decisions[decision].LL_MaxLook = LL_k;
            _decisions[decision].LL_MaxLookEvent = std::make_shared<LookaheadEventInfo>(
                decision, nullptr, alt, input, _startIndex, _llStopIndex, true);
        }
    }
    ...
    if (existingTargetState != nullptr)
    {
        ...
        if (existingTargetState == ERROR.get())
        {
            _decisions[_currentDecision].errors.push_back(
                ErrorInfo(_currentDecision, previousD->configs.get(), _input, _startIndex,
                          _sllStopIndex, false));
        }
    }
...
std::unique_ptr<ATNConfigSet> ProfilingATNSimulator::computeReachSet(ATNConfigSet *closure,
...
bool ProfilingATNSimulator::evalSemanticContext(Ref<const SemanticContext> const &pred,
                                                ...
                                                bool fullCtx)
{
    ...
    if (!(std::dynamic_pointer_cast<const SemanticContext::PrecedencePredicate>(pred) != nullptr))
    {
        ...
        _decisions[_currentDecision].predicateEvals.push_back(PredicateEvalInfo(
            _currentDecision, _input, _startIndex, stopIndex, pred, result, alt, fullCtx));
    }

    return result;
}

void ProfilingATNSimulator::reportAttemptingFullContext(DFA &dfa, const BitSet &conflictingAlts,
                                                        ...
                                                        size_t stopIndex)
{
    ...
    ParserATNSimulator::reportAttemptingFullContext(dfa, conflictingAlts, configs, startIndex,
                                                    stopIndex);
}

void ProfilingATNSimulator::reportContextSensitivity(DFA &dfa, size_t prediction,
                                                     ATNConfigSet *configs, size_t startIndex,
                                                     size_t stopIndex)
{
...
void ProfilingATNSimulator::reportAmbiguity(DFA &dfa, DFAState *D, size_t startIndex,
                                            size_t stopIndex, bool exact, const BitSet &ambigAlts,
                                            ATNConfigSet *configs)
{
    ...
    _decisions[_currentDecision].ambiguities.push_back(AmbiguityInfo(
        _currentDecision, configs, ambigAlts, _input, startIndex, stopIndex, configs->fullCtx));
