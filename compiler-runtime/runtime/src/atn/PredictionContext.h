    ...
    namespace atn
    {
        ...
        class ANTLR4CPP_PUBLIC PredictionContext
        {
          public:
            ...
            /// $ = EMPTY_RETURN_STATE.
            // ml: originally Integer.MAX_VALUE, which would be -1 for us, but this is already used
            // in places where
            //     -1 is converted to unsigned, so we use a different value here. Any value does the
            //     job provided it doesn't conflict with real return states.
            ...
            static Ref<const PredictionContext> mergeSingletons(
             ...
             */
            static Ref<const PredictionContext> mergeArrays(
            ...
            static Ref<const PredictionContext> getCachedContext(
                const Ref<const PredictionContext> &context, PredictionContextCache &contextCache);

            static std::vector<Ref<const PredictionContext>> getAllContextNodes(
                const Ref<const PredictionContext> &context);

            /// Convert a RuleContext tree to a PredictionContext graph.
            /// Return EMPTY if outerContext is empty.
            static Ref<const PredictionContext> fromRuleContext(const ATN &atn,
            ...
            PredictionContextType getContextType() const
            {
                return _contextType;
            }
