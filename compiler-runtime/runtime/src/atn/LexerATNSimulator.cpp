...
dfa::DFAState *LexerATNSimulator::computeTargetState(CharStream *input, dfa::DFAState *s, size_t t)
{
    OrderedATNConfigSet *reach =
        new OrderedATNConfigSet(); /* mem-check: deleted on error or managed by new DFA state. */
...
size_t LexerATNSimulator::failOrAccept(CharStream *input, ATNConfigSet *reach, size_t t)
{
    ...
    {
        accept(input, _prevAccept.dfaState->lexerActionExecutor, _startIndex, _prevAccept.index,
...
void LexerATNSimulator::getReachableConfigSet(CharStream *input, ATNConfigSet *closure_,
        ...
        { // for each transition
            ...
            if (target != nullptr)
            {
                ...
                Ref<LexerATNConfig> config = std::make_shared<LexerATNConfig>(
                    downCast<const LexerATNConfig &>(*c), target, std::move(lexerActionExecutor));
...
void LexerATNSimulator::accept(CharStream *input,
...
bool LexerATNSimulator::closure(CharStream *input, const Ref<LexerATNConfig> &config,
                                ...
                                bool speculative, bool treatEofAsEpsilon)
{
    ...
    if (config->state != nullptr && config->state->getStateType() == ATNStateType::RULE_STOP)
    {
        ...
        if (_recog != nullptr)
        {
            std::cout << "closure at " << _recog->getRuleNames()[config->state->ruleIndex]
        ...
        if (config->context == nullptr || config->context->hasEmptyPath())
        {
            ...
            {
                configs->add(std::make_shared<LexerATNConfig>(*config, config->state,
        ...
        if (config->context != nullptr && !config->context->isEmpty())
        {
            for (size_t i = 0; i < config->context->size(); i++)
            {
                if (config->context->getReturnState(i) != PredictionContext::EMPTY_RETURN_STATE)
                {
                    ...
                    Ref<LexerATNConfig> c =
                        std::make_shared<LexerATNConfig>(*config, returnState, newContext);
...
Ref<LexerATNConfig> LexerATNSimulator::getEpsilonTarget(CharStream *input,
                                                        const Ref<LexerATNConfig> &config,
                                                        const Transition *t, ATNConfigSet *configs,
                                                        bool speculative, bool treatEofAsEpsilon)
{
...
bool LexerATNSimulator::evaluatePredicate(CharStream *input, size_t ruleIndex, size_t predIndex,
                                          bool speculative)
{
