...
namespace
{
                                ...
                                const Ref<const PredictionContext> &rhs)
    {
        ...
        if (lhs == nullptr)
            return rhs == nullptr;
        if (rhs == nullptr)
            return false;    // lhs!=null and rhs==null
...
ArrayPredictionContext::ArrayPredictionContext(std::vector<Ref<const PredictionContext>> parents,
    ...
    : PredictionContext(PredictionContextType::ARRAY), parents(std::move(parents)),
      returnStates(std::move(returnStates))
{
...
bool ArrayPredictionContext::equals(const PredictionContext &other) const
{
    ...
    const bool sameSize =
        returnStates.size() == array.returnStates.size() && parents.size() == array.parents.size();
    if (!sameSize)
    {
        return false;
    }
    ...
    const bool returnStateArraysEqual = std::memcmp(returnStates.data(), array.returnStates.data(),
