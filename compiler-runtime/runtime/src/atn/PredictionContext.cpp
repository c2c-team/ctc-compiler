...
namespace
{
    ...
    Ref<const PredictionContext> getCachedContextImpl(
        const Ref<const PredictionContext> &context, PredictionContextCache &contextCache,
    ...
    size_t insertOrAssignNodeId(std::unordered_map<const PredictionContext *, size_t> &nodeIds,
...
const Ref<const PredictionContext> PredictionContext::EMPTY =
    std::make_shared<SingletonPredictionContext>(nullptr, PredictionContext::EMPTY_RETURN_STATE);
...
Ref<const PredictionContext> PredictionContext::fromRuleContext(const ATN &atn,
                                                                RuleContext *outerContext)
{
    ...
    auto parent = PredictionContext::fromRuleContext(
        atn, RuleContext::is(outerContext->parent) ? downCast<RuleContext *>(outerContext->parent)
                                                   : nullptr);
    const auto *transition = downCast<const RuleTransition *>(
        atn.states[outerContext->invokingState]->transitions[0].get());
    return SingletonPredictionContext::create(std::move(parent),
...
Ref<const PredictionContext> PredictionContext::merge(Ref<const PredictionContext> a,
                                                      ...
                                                      PredictionContextMergeCache *mergeCache)
{
    ...
    if (aType == PredictionContextType::SINGLETON && bType == PredictionContextType::SINGLETON)
    {
        return mergeSingletons(
            std::static_pointer_cast<const SingletonPredictionContext>(std::move(a)),
            std::static_pointer_cast<const SingletonPredictionContext>(std::move(b)),
...
Ref<const PredictionContext> PredictionContext::mergeSingletons(
    Ref<const SingletonPredictionContext> a, Ref<const SingletonPredictionContext> b,
    bool rootIsWildcard, PredictionContextMergeCache *mergeCache)
{
    ...
    if (a == b || (*parentA == *parentB))
    { // ax + bx = [a,b]x
        singleParent = parentA;
    }
    if (singleParent)
    { // parents are same, sort payloads and use same parent
...
Ref<const PredictionContext> PredictionContext::mergeRoot(Ref<const SingletonPredictionContext> a,
...
Ref<const PredictionContext> PredictionContext::mergeArrays(Ref<const ArrayPredictionContext> a,
                                                            ...
                                                            PredictionContextMergeCache *mergeCache)
{
    if (mergeCache)
    {
        ...
        {
#if TRACE_ATN_SIM == 1
            std::cout << "mergeArrays a=" << a->toString() << ",b=" << b->toString()
                      << " -> previous" << std::endl;
#endif
            return existing;
        }
    ...
    std::vector<Ref<const PredictionContext>> mergedParents(a->returnStates.size() +
                                                            b->returnStates.size());
    ...
    if (m == *a)
    {
        ...
        {
#if TRACE_ATN_SIM == 1
            std::cout << "mergeArrays a=" << a->toString() << ",b=" << b->toString() << " -> a"
                      << std::endl;
#endif
    ...
    if (m == *b)
    {
        ...
        {
#if TRACE_ATN_SIM == 1
            std::cout << "mergeArrays a=" << a->toString() << ",b=" << b->toString() << " -> b"
                      << std::endl;
#endif
    ...
    for (const auto &current : nodes)
    {
        ...
        for (size_t i = 0; i < current->size(); i++)
        {
            ...
            ss << "  s" << insertOrAssignNodeId(nodeIds, nodeId, current.get()) << "->"
               << "s" << insertOrAssignNodeId(nodeIds, nodeId, current->getParent(i).get());
            if (current->size() > 1)
            {
                ss << " [label=\"parent[" << i << "]\"];\n";
            }
...
// recursive).
Ref<const PredictionContext> PredictionContext::getCachedContext(
    const Ref<const PredictionContext> &context, PredictionContextCache &contextCache)
{
    std::unordered_map<Ref<const PredictionContext>, Ref<const PredictionContext>> visited;
