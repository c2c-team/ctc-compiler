...
void DiagnosticErrorListener::reportAmbiguity(Parser *recognizer, const dfa::DFA &dfa,
                                              ...
                                              atn::ATNConfigSet *configs)
{
    ...
    std::string message = "reportAmbiguity d=" + decision +
                          ": ambigAlts=" + conflictingAlts.toString() + ", input='" + text + "'";
...
void DiagnosticErrorListener::reportContextSensitivity(Parser *recognizer, const dfa::DFA &dfa,
...
antlrcpp::BitSet DiagnosticErrorListener::getConflictingAlts(const antlrcpp::BitSet &reportedAlts,
                                                             atn::ATNConfigSet *configs)
{
    if (reportedAlts.count() > 0)
    { // Not exactly like the original Java code, but this listener is only used
